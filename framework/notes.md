...
*Last updated: 2026-05-02 (rev. 25)*
...
| [`sparring-monte-carlo-design.md`](sparring-monte-carlo-design.md) | DRAFT rev. 2 | Eval-harness design bridging Appendix D Phase 2/3 deferred work into a concrete milestone shape, modeled on the combat-engine Monte Carlo machinery. Drafted 2026-05-02; revised same day after two pressure-test spars (`spar-low-cost-pilot-design`, `spar-historical-case-corpus`) surfaced operational constraints invisible from methodology alone. Now includes: 4 candidate corpus paths with honest current-status assessments; corrected $40-60k contracted-labor cost picture (compute is rounding error against rater labor); explicit lifspel-scale operational constraints section; defensibility rubric of 5 disciplines for any future eval path; reordered phasing (Phase A executable now with publishability-ceiling fence; Phases B-E deferred pending labor-pool resolution; new Phase E historical-case-corpus contingent on academic partnership). Not yet a milestone. |
| [`sparring-llm-judge-pilot-design.md`](sparring-llm-judge-pilot-design.md) | DRAFT | Comparative-evidence pilot methodology that uses cross-vendor LLM judges (Claude + GPT + Grok) as the structural skeptic, sidestepping the operational impossibility of human adversarial co-curators. Surfaces from partner observation that both prior spar agents had missed LLM-as-judge as a third evidence base. Historical-case corpus (10-15 cases) WITHOUT hindsight stripping; comparative question is "which deliberation is higher quality?" not "is the answer correct?"; output normalization removes framework-shaped scaffolding before judging; partner-anchored calibration step gates the full pilot. Cost: $100-300 cash + 100-200 partner hours over 1-2 months. Publishable as exploratory evidence under ASA framing, not RCT-grade. Two-phase execution: Phase 1 calibration check (~$10-20 cash + 5-10 partner hours) before committing to Phase 2 full pilot. Drafted 2026-05-02. |

---

*Revision history:*
*- 2026-05-02 r25 -- added "Pack-fidelity without ground-truth-fidelity" to the failure-mode catalog. First observed during the 2026-05-02 SPARRING LLM-judge pilot Case B (artifact: `docs/bfn/llm-judge-pilot-2026-05-02/preprint-draft.md` §5.4): the case-b decision pack asserted a `{walk, trot, canter, gallop}` engine gait enum that did not exist; both Generator (engine-architect persona, with the engine codebase as evidence base) and Challenger (historical-biomechanics persona) accepted the assertion as fact without verifying; partner Matthew Niedner caught the error post-hoc. The verifiable-artifact requirement (D3) governs an agent's *outgoing* citations but does not impose source-fetch-and-verify on *incoming* assertions inherited from the pack or asserted by the agent about their own evidence base. Operational defense (Layer 2) added: Challenger should explicitly flag foundational assumptions the Generator inherits but does not verify, extending the existing self-citation-circularity tagging discipline (`internally consistent / externally unvalidated`) one layer outward. Structural fix (Layer 1, mandate Generator fetch-and-verify) deferred pending more evidence -- the rule should not be hardened from a single observation; observe across 2-3 future spars before promoting. Partner residual safety remains load-bearing while this gap exists. No /spar SKILL.md update yet; Layer 2 defense is a substrate observation that informs partner judgment without yet imposing a discipline rule.*
*- 2026-05-02 r24 -- new sibling working draft `sparring-llm-judge-pilot-design.md` capturing a comparative-evidence pilot methodology that uses cross-vendor LLM judges (Claude + GPT + Grok) as the structural skeptic. Surfaced when partner observation pointed out that both prior spar agents (`spar-low-cost-pilot-design`, `spar-historical-case-corpus`) had missed LLM-as-judge as a third evidence base outside both their disjoint reading lists. The new design: historical-case corpus WITHOUT hindsight stripping; comparative question reframed from "is the answer correct?" to "which deliberation is higher quality?"; output normalization removes framework-shaped scaffolding before judging; partner-anchored calibration step gates the full pilot. Cost $100-300 cash + 100-200 partner hours over 1-2 months. Two-phase execution: Phase 1 calibration check (~$10-20 cash + 5-10 partner hours) before committing to Phase 2 full pilot. Methodology notes captured at end of doc on "why both prior spar agents missed this" -- disjoint-evidence specialization can miss something important when the right answer requires a third evidence base neither agent has; partner judgment after the spar is itself a third evidence base; "always run another spar" is not always the right response to partial-convergence outcomes. Sibling-drafts table updated.*
*- 2026-05-02 r23 -- added "Substrate independence" subsection between "Variants and operational patterns" and "The nine disciplines." Captures the explicit claim that the framework's structural commitments are model-agnostic within a sufficiently-capable class -- naming what changes across substrate (cost, magnitude of leverage, prominence of failure modes), what does not (structural argument, disciplines, protocol), the falsifiable form of the claim (Phase D cross-model integration would test it directly), and the practical consequences (eval harness work doesn't require Opus; future model deprecations don't invalidate prior artifacts; the framework would have been the same on Sonnet 3.5 or GPT-4 if built earlier). The triggering discussion: partner observation that costs were calculated against Opus, surfacing the question of whether the framework's leverage was model-dependent. Substrate independence is what makes the "method, not a model integration" positioning coherent; making the claim explicit is the discipline against pleasing-bias drift toward "use Opus and these prompts" framing.*
...
The framework's claims are **structural, not model-dependent**. The structural commitments -- two roles with role-shifting Challenger function across four phases, disjoint evidence (D2), verifiable artifacts (D3), both-must-agree convergence (D4), the disagreement-at-cap response protocol -- are claims about *how to organize multi-agent decision-making*. They are not claims about a specific vendor, model family, generation, or capability tier.

**The sufficiently-capable boundary is real.** The framework cannot operate on a model too small or too weakly-instruction-following to maintain persona discipline across rounds, ground citations in evidence bases reliably, or produce structured agreement signals on demand. This is the model-ceiling boundary named in "What the framework does not address" -- you cannot multi-agent your way past underlying capability. Within frontier-class models (current Claude family, GPT-4 class, recent Gemini, Grok-4 class), the framework's structural argument propagates without modification.

**What changes when the substrate changes:**
- **Cost.** Per-spar cost varies dramatically across models: Opus ~$1/spar, Sonnet ~$0.20/spar, Haiku less, GPT-4-class roughly Sonnet-comparable, Grok roughly Sonnet-comparable. Substrate choice is a budgeting decision, not a methodology decision.
- **Magnitude of measured leverage.** A model with stronger pre-baked sycophancy reduction benefits *less* from D3's verifiable-artifact discipline because it's already doing some of that work. A weaker model benefits more. The ceiling-and-variance framing in "Conditionalities" applies: SPARRING reduces single-shot variance toward the model's ceiling, so a higher ceiling means less marginal gain from the framework but the same shape of contribution.
- **Prominence of specific failure modes.** Different RLHF approaches across vendors produce different sycophancy patterns. Anthropic's Constitutional AI, OpenAI's RLHF, and xAI's training each leave different residual failure-mode shapes. The framework's failure-mode catalog is a superset; which specific items are most prominent shifts by vendor.

**What does NOT change:**
- The structural argument that disjoint cross-evidence challenge with both-must-agree convergence is more robust than single-agent.
- The nine operational disciplines and their function.
- The disagreement-at-cap response menu.
- The verifiable-artifact and self-citation circularity rules.
- The applicability boundaries (model ceiling, pure-judgment topics, low-stakes routine work).

**Cross-model SPARRING (Substrate variants section above) is the framework's deliberate design move addressing this claim.** Same-vendor pairings satisfy Discipline 2 at the evidence-base layer but share model substrate (training distribution, RLHF approach, sycophancy-reduction tuning, reasoning-pattern biases). Cross-model pairings extend the disjointness one layer down, reducing the residual coordinated-theater risk that same-substrate pairings carry. The variant exists *because* the framework's claim should be robust across the substrate axis -- and providing a structural mechanism to test that robustness is part of the framework's discipline against itself.

**Falsifiability.** What WOULD nullify the substrate-independence claim is empirical evidence that, when the same SPARRING ceremony is run on different model families against the same decision corpus, the leverage manifests *only* on one specific vendor or model class. That would be evidence the "leverage" was actually model-specific behavior (e.g., Opus-quirks in following persona instructions) rather than the structural cross-evidence challenge the framework theorizes. The Phase D cross-model integration in `sparring-monte-carlo-design.md` is the eval-harness step that would test this directly. Until that eval runs, substrate independence is a structurally-argued claim, not an empirically-validated one. The framework's discipline against pleasing-bias-toward-its-own-claims requires naming this honestly.

**Practical consequences:**
- The `/spar` skill's current `model: "opus"` default is a deployment choice (one vendor, one tier), not a methodology requirement. Per-spar model selection is a configuration decision, not a framework decision.
- Eval-harness work (Phase A variance checking, Phase B comparative testing) does not require Opus. Running on Sonnet or Haiku at lower cost, or cross-model at higher robustness, is methodologically sound.
- Future model deprecations (Opus 4.7 -> 4.8 -> 5.x; Sonnet generation rolls; vendor-level changes) do not invalidate prior spar artifacts as evidence -- they may shift the substrate but not the structural claim. Older artifacts remain valid as records of "what the ceremony produced on this substrate at this date."
- If the framework had been built before Opus existed (2024 or earlier), it would have been built on Sonnet 3.5 or GPT-4. The structural claim and disciplines would be identical. The cost picture and the magnitude of measured leverage would differ; the framework's thesis would not.

**Where this lives in the framework's positioning.** SPARRING is positioned as a method for designing multi-agent decision systems, not as an integration of a specific LLM. The substrate-independence claim is what makes that positioning coherent: a method whose claims depended on a specific model would be a model integration with extra steps, not a framework. The discipline of stating this explicitly -- and naming the falsifiable form -- is part of refusing the pleasing-bias drift that would otherwise let the framework quietly become "use Opus and these prompts" while presenting as something more general.
...
- **Self-citation circularity** -- a claim cited as evidence where the citation refers to the topic-owner's own working notes or specification. The citation is verifiable in the literal sense (the cited text exists and says what it claims) but not externally validated -- the same author authored both the claim and the evidence for it. Most likely to surface when the spar topic IS the framework itself, or when an agent's primary evidence base is the topic-owner's own materials. The verifiable-artifact requirement (D3) catches manufactured concerns but does not by itself distinguish externally-corroborated artifacts from author-internal ones; the operational defense is to tag such claims as `internally consistent / externally unvalidated` rather than treating them as load-bearing, with the structural fix awaiting the empirical validation work in Appendix D Phase 1.
- **Pack-fidelity without ground-truth-fidelity** -- agents accept the input pack's assertions about external systems (engine state, schema, code structure, documented behavior) as ground truth and build load-bearing claims on top of them, without fetching-and-verifying the cited artifact. The verifiable-artifact requirement (D3) governs the agent's *outgoing* citations -- "every concern you raise must cite a specific artifact" -- but does not by itself impose source-fetch-and-verify on the *incoming* assertions the agent inherits from the pack or makes about their own evidence base. The Generator can ground in "the engine has X structure" without ever opening the engine file; the Challenger can pressure-test the recommendation without verifying the foundational claim it rests on, especially when the foundational claim is outside the Challenger's evidence base. First observed during the 2026-05-02 SPARRING LLM-judge pilot Case B (artifact: `docs/bfn/llm-judge-pilot-2026-05-02/preprint-draft.md` §5.4): the Case B decision pack asserted a `{walk, trot, canter, gallop}` gait enum that did not exist in the engine; both Generator (engine-architect persona) and Challenger (historical-biomechanics persona) accepted the assertion as fact; the partner caught the error post-hoc by reading the pack with the engine source available. Operational defense (Layer 2): the Challenger should explicitly flag foundational assumptions the Generator inherits or asserts but does not verify ("Generator's recommendation rests on the asserted fact that X; I cannot verify this from my evidence base -- partner verification recommended"). This shifts the existing self-citation-circularity tagging discipline (`internally consistent / externally unvalidated`) one layer outward to cover pack-asserted facts as well as author-internal materials. Structural fix (Layer 1, more invasive, deferred pending more evidence): require the Generator to fetch-and-verify any load-bearing factual claim about their cited evidence base before grounding the recommendation in it. The Layer 1 cost is real (additional Read calls per spar) and the rule should not be hardened from a single observation; observe across 2-3 future spars before promoting. The framework's residual safety (partner judgment of surfaced artifacts) remains load-bearing while this gap exists.
